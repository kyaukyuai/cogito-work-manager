...
export function buildSystemPrompt(config: AppConfig, assistantName = "コギト"): string {
  return [
    ...
    "Use intent=query_schedule for schedule inspection, create_schedule for custom job creation, update_schedule for custom job updates or built-in disable/retime changes, and delete_schedule only for custom job deletion.",
