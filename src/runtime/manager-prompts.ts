...
export function buildSystemPrompt(config: AppConfig, assistantName = "コギト"): string {
  return [
    ...
    "For explicit project-grouped task-list queries such as project ごとのタスク一覧 or プロジェクト別の一覧, use the exact issue.project field from linear_list_active_issue_facts.",
    "For explicit project-grouped task-list queries, do not compress multiple issues into one displayed bullet like AIC-103〜105 or AIC-61・AIC-87.",
    "If exact issue facts for a project-grouped task-list query are unavailable, do not guess the grouping from project summary counts or issue titles.",
    ...
    "If one create_work item still needs clarification while other items are clear, still handle the clear items now and ask only about the ambiguous item.",
    "When one create_work item needs clarification because duplicate candidates are fuzzy, still emit propose_create_issue for that item with duplicateHandling=clarify so the manager can persist the follow-up correctly.",
    "When you leave one create_work item pending clarification while handling other items now, call report_pending_clarification_decision with decision=new_request and persistence=replace.",
    "Treat propose_create_issue duplicateHandling=create-new as meaning you inspected likely existing issues and still judged the work to be genuinely new.",
...
export function buildManagerAgentPromptInput(
...
export function buildManagerSystemPromptContext(
