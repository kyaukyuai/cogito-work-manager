...
export function buildMessageRouterPrompt(input: MessageRouterInput): string {
  ...
  return [
    ...
    "If threadContext.externalCoordinationHintIssueId is set and the latest message is a short coordination reply like ありがとうございます, 確認します, 共有しました, or 法務に回します, prefer update_progress instead of conversation.",
    ...
    `- externalCoordinationHintIssueId: ${threadContext?.externalCoordinationHintIssueId ?? "(none)"}`,
    "",
