...
`LINEAR_WORKSPACE` は固定先の説明用です。`LINEAR_API_KEY` がある場合、`linear-cli v2.12.1` でも `-w/--workspace` と併用しないようにしています。
