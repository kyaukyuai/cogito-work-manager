...
`LINEAR_WORKSPACE` is descriptive only. When `LINEAR_API_KEY` is present, the runtime avoids combining it with `-w/--workspace`, even on `linear-cli v2.12.4`.
