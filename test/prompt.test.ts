...
describe("prompt helpers", () => {
  it("states Slack-first execution rules in the system prompt", () => {
    ...
    expect(prompt).toContain("If one create_work item still needs clarification while other items are clear, still handle the clear items now and ask only about the ambiguous item.");
    expect(prompt).toContain("When one create_work item needs clarification because duplicate candidates are fuzzy, still emit propose_create_issue for that item with duplicateHandling=clarify so the manager can persist the follow-up correctly.");
    expect(prompt).toContain("When you leave one create_work item pending clarification while handling other items now, call report_pending_clarification_decision with decision=new_request and persistence=replace.");
