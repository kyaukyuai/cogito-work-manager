...
- As of 2026-04-06, the Linear runtime contract assumes `linear-cli v3.1.0`, accepts additive `linear capabilities --json` schema changes, verifies `LINEAR_TEAM_KEY` through `team list --json`, and preserves `timeout_error.appliedState` and `callerGuidance` for repo-side reconciliation
