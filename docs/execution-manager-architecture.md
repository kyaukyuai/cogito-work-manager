...
Linear issue create webhook の execute / no-op 判定は、「即時実行価値」ではなく「既存 proposal surface で安全に実行できる action があるか」を基準にする。no-op は silent とし、committed / failed のみ control room に通知する。

Slack mention は `operator-directed` な明示投稿と、review / heartbeat の `system follow-up` 通知を分けて扱う。前者は manager commit で明示依頼された 1 件の投稿だけを扱い、後者は control room の follow-up request に限って担当者を 1 回だけメンションする。summary / webhook status 通知ではメンションしない。
